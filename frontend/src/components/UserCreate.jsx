...
import { Plus, Search, Filter, Phone, Mail, UserCheck, Edit, Trash2, DollarSign, Send, Wallet } from 'lucide-react';
...
export const UserCreate = () => {
  ...
  // 🐒 পপআপে Available Balance সহ পে রিকোয়েস্ট উইন্ডো
  const handlePayUser = async (user) => {
    const targetUserId = user._id || user.id;
    const userBalance = Number(user.totalAmount || user.totalDollar || user.dollar || user.balance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    try {
      ...
      if (res.ok && data.success !== false) {
        Swal.fire({
          ...
          text: 'Withdrawal request has been submitted to Master Admin.',
  ...
  return (
    <div className="p-3 sm:p-6 bg-gray-50/50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        {canManageUsers && (
          <button
            ...
            className="bg-green-500 hover:bg-green-600 text-white px-4 py-2.5 rounded-xl font-bold shadow-md shadow-green-100 flex items-center gap-2 transition-all transform hover:scale-[1.02] cursor-pointer text-xs sm:text-sm"
          >
      ...
      <div className="bg-white p-3 sm:p-4 rounded-2xl border border-gray-100 shadow-sm mb-5 flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full md:w-52 p-2.5 bg-gray-50/80 rounded-xl border border-gray-200 outline-none text-xs sm:text-sm cursor-pointer font-medium text-gray-700 focus:border-green-500 focus:bg-white transition-all"
          >
          ...
          <table className="w-full text-left border-collapse min-w-[750px]">
            <thead>
              <tr className="bg-gray-50/70 text-gray-400 text-[10px] sm:text-[11px] uppercase tracking-wider border-b border-gray-100">
                ...
                <th className="py-3 px-4 font-bold">Total Dollar</th>
                  ...
                  <tr key={user._id || user.id} className="hover:bg-green-50/20 transition-colors group">
                    <td className="py-3 px-4 font-semibold text-gray-800">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-green-100 text-green-600 rounded-xl flex items-center justify-center font-bold text-xs shadow-2xs group-hover:scale-105 transition-transform shrink-0">
                          {user.name ? user.name.charAt(0).toUpperCase() : 'U'}
                        </div>
                        <div className="min-w-0">
                          <div className="font-bold text-gray-900 truncate">{user.name}</div>
                          <div className="text-[10px] text-gray-400 font-normal truncate max-w-[120px]">
                    ...
                    <td className="py-3 px-4 text-gray-600">
                      <span className="inline-flex items-center gap-1 bg-gray-50 border border-gray-100 px-2.5 py-1 rounded-lg text-[11px] font-semibold text-gray-700">
                        <Phone size={11} className="text-green-500 shrink-0" /> {user.whatsapp || 'N/A'}
                      </span>
                    </td>
                    <td className="py-3 px-4 font-bold text-gray-700">
                      <span className="inline-flex items-center gap-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-lg text-xs font-bold">
                        <DollarSign size={13} />
                        {Number(user.totalAmount || user.totalDollar || user.dollar || user.balance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <span
                        className={`inline-flex px-2.5 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
      ...
      {isModalOpen && canManageUsers && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md p-6 sm:p-7 rounded-2xl shadow-2xl relative">
